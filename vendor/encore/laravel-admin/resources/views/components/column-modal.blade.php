...
<script>
    ...
    var load = function (url) {
        ...
        $.get(url, function (data) {
            modalBody.html(data);
            if (modalBody.find('.custom-modal-title').length > 0) {
              modal.find('.modal-title').text(modalBody.find('.custom-modal-title').text());
              modalBody.find('.custom-modal-title').hide();
            }
            if (modalBody.find('.direct-chat-messages').length > 0) {
              $(window).on('resize', function () {
                windowH = $(window).height();
                modalBody.find('.direct-chat-messages').css('height', windowH - 145);
              });
