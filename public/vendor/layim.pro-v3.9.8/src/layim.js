...
layui.define(['layer', 'laytpl', 'upload'], function (exports) {
    ...
    //重写显示聊天记录
    LAYIM.prototype.viewServerChatlog = function (content) {
        return viewServerChatlog(content);
    };
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++;console.log(data) }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" class="layim-'+ (options.type === 'history' ? '{{i}}' : options.type + '{{data.id}}') +' {{ data.status === "offline" ? "layim-list-gray" : "" }}"><img src="{{ data.avatar || layui.cache.layimAssetsPath + \'images/default.png\'}}">'
      ,'<span>{{ data.username||data.groupname||data.name||"佚名" }}</span>'
    //   ,'<span style="float:right;display:none" class="weidu">{{ data.lasttime||"" }}</span>' 
      ,'<br><span style="color:#ddd">{{ data.remark||data.sign||"" }}</span><span class="layim-msg-status">new111</span>'
      ,'<span class="layui-badge weidu" style="display:none" lay-type="hide">{{ data.weidu||"" }}</span>'
      ,'</li>'
    ...
    var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
        ...
        , '{{# }); }}'
        , '<select class="quick" layim-event="quick">'
        , '<option value="q">快捷回复</option>'
        , '{{# layui.each(d.base.quick, function(index, item){ }}'
        , '<option class="layui-icon" value="{{item.id}}" data-index="{{ d.data.id }}">{{item.title}}</option>'
        , '{{# }); }}'
        , '</select>'
    ...
    var forwardChat = function (elem, forwardMsg) {
        elem = elem || $('.layim-chat-list .' + THIS);
        var index = elem.index() === -1 ? 0 : elem.index();
        var str = '.layim-chat', cont = layimChat.find(str).eq(index);
        cont.find('textarea').remove();
        forwardMsg = forwardMsg ? forwardMsg : '';
        cont.find(".layim-chat-textarea").append('<textarea>' + forwardMsg + '</textarea>');
    };
    ...
    //渲染服务端最新聊天记录到相应面板
    var viewServerChatlog = function (serverChatlog) {
        //获取当前选中的聊天面板，并将服务端的聊天记录渲染到面板
        var thatChat = thisChat(), ul = thatChat.elem.find('.layim-chat-main ul');
        //清空面板的聊天记录
        ul.empty();
        //渲染聊天记录到面板
        layui.each(serverChatlog[thatChat.data.type + thatChat.data.id], function (index, item) {
            ul.append(laytpl(elemChatMain).render(item));
        });
    };
    ...
    var anim = 'layui-anim-upbit', events = {
        ...
        , quick: function (othis) {
            othis.on('change', function (a) {
                var text = othis.find("option:selected").text();
                var id = othis.find("option:selected").attr("data-index");
                forwardChat($('.layim-chatlist-group' + id), text);
            });
        }
    };
